    ...
    const loadAlarmsData = async () => {
        try {
            ...
            if (result.alarms && Array.isArray(result.alarms) && result.alarms.length > 0) {
                ...
                const mappedAlarms = retamarAlarms.map((alarm, index) => ({
                ...
                if (!result.success) {
                    ...
                    setError('No se encontraron alarmas en Retamar');
                }
            }
        } catch (error) {
        ...
        return (
            <div className="w-full p-6 bg-panel rounded-lg">
                <div className="flex items-center justify-center h-32 space-x-3">
                    ...
                    <span className="text-secondary">Cargando alarmas de Retamar...</span>
                </div>
            </div>
        );
    };
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h1 className="text-2xl font-semibold text-primary">Alarmas - Retamar</h1>
                {error && (
                ...
                {alarms.length === 0 && !loading && (
                    <div className="text-center py-8">
                        ...
                        <p className="text-secondary">
                            No hay alarmas activas en Retamar
                        </p>
                    </div>
                )}
            </div>
...
export default RetamarAlarmsTable;